...
| GPU | Not required. Leave the node devices on `auto` and assign the graph's device from the toolbar (or Settings). The per-node `device` parameter is kept for older graphs. |
