...
| `execute(self, inputs, params, progress_callback=None, *, context=None)` | Execute the node and return a dict keyed by output-port name. The engine passes each optional keyword argument only when the signature declares it. `progress_callback` receives a dict for each progress event; for example, the training loop sends `{"event": "epoch", ...}`. While the node runs, a `text` string in the dict is shown on the node card, except in an `epoch` event. Each event replaces the last and the card shows at most the text's last 1,000 characters, so send the end of the text so far (`progress_text_tail` from `app.core.loop_control`), not only the newest part. Events are also stored with the run, so rate-limit frequent ones with `ProgressThrottle` from the same module. `context` provides the run's device, seed, and determinism flag. |
