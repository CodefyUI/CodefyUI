...
Educational ("Edu") nodes ship as installable **plugin packs**, organised **by direction** so each maps onto a hands-on textbook module and installs cumulatively as you progress. The editor's [Plugin Center](/usage/plugin-center) has buttons for installing, enabling, disabling, updating and uninstalling; `sync`, `info` and `search` are command-line only. This page is the reference behind both.
...
- **Third-party packs** are downloaded as a pinned-SHA tarball into `<USER_DATA>/plugins/<id>/` and **AST-validated** before install (see [Security](#security--three-tiers)). The tarball is the whole repository at that commit. An install refuses a tarball above 100 MB compressed or 500 MB unpacked and a `cdui.plugin.toml` above 1 MB, and keeps nothing after a refusal, so keep datasets and model weights out of the repository. Archive members that would be written outside the unpack directory (such as `../` paths), links that point outside it, and device files are also refused.
...
**Opening the Plugin Center.** Use either the **Plugin Center...** button in the **Plugins** section of the sidebar's **Custom & Plugins** tab or the **Open** button on the **Plugin Center** row under **Settings → Optional Packs & Plugins**. That row reads "Install teaching node packs and plugins from GitHub." until the catalog has loaded and "*N* installed, *M* available" after it; while an install runs it reads "Installing *name*...", and on a server without a Plugin Center "Not available on this server". The Package Center and Plugin Center can remain open at the same time; **Escape** closes the topmost window.

**Plugin list.** The list contains every built-in or official plugin available by catalog name and every installed plugin. Filters are **All**, **Installed**, and **Not installed**. Each card shows the name, status, version, origin, repository and pin (`ref @ sha`), lessons, node count, and Python dependencies. Origin is **Built-in**, **Official**, or **Linked folder** for a directory registered with `cdui plugin link`; other third-party repositories have no origin chip. Available actions depend on the [install state](#install-states).

**Installing a plugin outside the catalog.** Open the folded **Install from GitHub** section above the list, enter `owner/repo`, `owner/repo@ref`, a GitHub URL, or a catalog name, then select **Review**. Other formats are rejected before submission with "Enter owner/repo[@ref] or a GitHub URL."

**Review and consent.** Selecting **Install** on a card or **Review** in the input reads the manifest at one resolved commit. When consent is required, a **Review before installing** card appears at the top of the list. It shows the name, version, description, author, the nodes the catalog lists for that id (fresh installs only; a GitHub plugin that is not installed yet lists none, because naming its nodes would require importing it), Python dependencies, commit pin, and an HTTP or HTTPS **Homepage** link when provided. Each required decision has a checkbox. **This plugin asks for:** lists each declared capability and its access; **Grant these capabilities** records [Tier 1](#security--three-tiers) consent. **Trust this author to import ...**, followed by the module names, lists `allowed_modules` and records [Tier 2](#security--three-tiers) consent. Browser code adds the warning "Ships JavaScript that runs in this editor with full access." **Install** remains disabled until all required checkboxes are selected. On an update review the button reads **Update**. Built-in packs require no consent and install directly from their row. An installed plugin displays a replacement warning and a **Reinstall** button.
...
**Using another computer.** When the server is not bound to a loopback address, the footer reads "Installing works only from the computer that runs the server." **Review**, **Install**, **Update**, and **Uninstall** are disabled with the same tooltip; **Enable** and **Disable** remain available. **Cancel install** stays enabled, but the server refuses it. Set `CODEFYUI_ALLOW_REMOTE_PLUGIN_INSTALL=1` to permit remote plugin operations. See [How an install runs](#how-an-install-runs) for the affected routes. A server older than the panel, or one whose plugin service did not start, shows "This server has no Plugin Center. Update CodefyUI and restart."
...
| `removed` | Removed | No lockfile entry, but a `removed` record from an uninstall, so `cdui plugin sync` leaves it alone. Counted under **Not installed**. | Install (clears the record) |
...
**Plugin installation depends on the server's bind address.** The inspect, install, cancel, update, and delete routes require the session token and a server bound to a loopback address. These routes can retrieve external code, install it into the server process, or remove an installed plugin. The gate checks the address the server is bound to, never where a request comes from: behind a reverse proxy on a loopback bind, everyone who reaches the editor can install (see [Deployment](/usage/deployment#authentication-is-the-proxys-job-and-it-has-limits)). A classroom or lab server on a LAN can allow remote access with `CODEFYUI_ALLOW_REMOTE_PLUGIN_INSTALL=1`. Reload, enable, and disable require the token but not a loopback binding because they operate on existing local files. Read routes remain open, including job events, so another tab can monitor an active installation.

**GitHub API rate limits.** Unauthenticated GitHub API access is limited to 60 requests per hour for each IP address. Computers behind a shared NAT use the same quota. When the quota is exhausted, the panel shows "GitHub's request limit was reached" (`502` `github_rate_limited`). Set `CODEFYUI_GITHUB_TOKEN` to a token with read access to public repositories. For the Plugin Center, set it in the environment `cdui start` runs in (or, for a server started on a project directory, in that project's `.env`) and restart the server: a running server does not see a variable exported later in another shell. `cdui plugin install|info|update` read it from the shell they run in, so exporting it there needs no restart. The token is read for each GitHub request, sent only to GitHub as a bearer header, removed on redirects, and excluded from logs and error messages.

**The Plugin Center names the current step in the interface language.** Its log, the CLI's step lines (`Resolving …`, `Downloading …`, `Unpacking …`, `Scanning … for unsafe code`, `Installing packages: …`, `Installing …`, `Recording …`), and the backend's refusal and failure messages and hints stay in English.
...
| 400 | `reserved_id` | inspect, update | The manifest `id` is reserved: by a route name such as `catalog` (`holder` is `"route"`), by a built-in pack (`"builtin_pack"`), or by a catalog entry that names a different repository (`"repository"`). The response includes `id`, `holder` and, for `"repository"`, that entry's `repo`. |
...
| 503 | `unavailable` | inspect, install, events, cancel, update | The plugin service did not start. `GET /catalog` remains available, and the panel shows "This server has no Plugin Center. Update CodefyUI and restart." |
...
| `[security] allowed_modules` | yes | A list of module names — [Tier 2](#security--three-tiers), installable only with `--trust-author` or the review card's "Trust this author to import ..." box. A bare string instead of a list is refused. |
...
`link` reads the id from your `cdui.plugin.toml` and records the directory's absolute path in the lockfile as `source_kind = "local"`, so discovery walks your working tree directly. The AST security gate is skipped for linked plugins (it's your own code, and a warning says so); `unlink` drops only the lockfile entry, never your files. After editing Python nodes, `cdui plugin reload` (or `cdui plugin dev`) re-imports them on the server; press **Reload Nodes** in the toolbar, or reload the page, to see new or changed nodes in the palette. **Frontend edits reload automatically** when the page was opened while an enabled linked plugin was installed: the editor then checks for a reload every 1.5 seconds and re-activates the plugin UIs after each one. After linking a plugin into an editor that is already open, reload the browser once.

A linked plugin's `[python_deps]` are installed by the same rules a downloaded pack's are: add-only, under the constraints file that pins every package installed in the server's environment. So `cdui plugin link` has the install path's exit codes as well — `3` when a package it asks for cannot go into a live server (the command to run with the server stopped is printed), and `130` for a `Ctrl+C` — where it used to hand back whatever the package manager returned.
...
| `/api/plugins/generation` | GET | open | Return the reload counter. Every rediscovery of nodes and plugins increments it, including an install, update, uninstall, enable, disable, or reload. The editor polls it every 1.5 seconds only when an enabled linked plugin was installed at page load, and re-activates plugin frontends when it changes; re-imported bundles use it as a cache-buster. |
...
| `/api/plugins/jobs/{job_id}/events` | GET | open | Return install-job events after `?cursor=` (default `0`), at most `?limit=` of them (1 to 2000, default 500); `?wait=` (0 to 60 seconds) long-polls when there is nothing new. The response is `{job_id, status, events, cursor}`. |
