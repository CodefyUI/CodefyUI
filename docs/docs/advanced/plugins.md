...
**An install can end in `needs_restart`, which is not a failure.** A plugin's `[python_deps]` are installed add-only, under a constraints file that pins every package the running server has already loaded, so nothing a plugin asks for can downgrade what your session is holding open. When the resolver says that cannot be done live, the job ends `needs_restart` and carries the exact `command` to run with the server stopped. Nothing is wrong with the plugin, and asking the same server again will end the same way. (`cdui plugin install` prints that command too, and exits `3`.)
