...
**Reachable subpackages** are allowlisted one by one (`TIER0_MODULE_PATHS`), not implied by their root:
...
Everything else under those roots — `torch.utils`, `torch.package`, `torch.fx`, `torch.serialization`, `numpy.f2py`, `numpy.testing`, `numpy.lib.format` — is refused, because each of them held a one-line escape. If a recipe you need is missing one, that is a deliberate line rather than an oversight: say so, and it can be weighed on its own.
...
* **Private and dunder attributes of a library**: `re._parser`, `statistics._sum`, `numpy.__version__`. A library's private names are exactly where its own imports live. Use the public API instead — but note that a library's **version is not reachable at all** under Tier 0: `torch.__version__` and `numpy.__version__` are dunders, and `torch.version` / `numpy.version` are modules that the subpackage allowlist does not carry. A script that has to branch on a library version belongs in a [custom node](./custom-nodes.md).
...
* **Values that are capabilities rather than data**: a `pathlib.Path` (class or instance), an open file object, an `mmap`, an `os.DirEntry` — and anything defined by a module the policy blocks, such as `subprocess.Popen` or `importlib.find_spec`, however an allowed library re-exports it. The check is on the value's type, so a subclass or an alias under any name is covered.

Plain values otherwise come back **unwrapped**: `torch.zeros(3)` is an ordinary tensor, not a proxy. That is deliberate — proxying the data as well as the module surface would put a Python-level check in front of every `.mean()` in every script. It is also the shape of the remaining residual: see the security section.
...
* `exec`, `eval`, `compile`, `__import__`, `open`, `input`, `globals`, `locals`, `vars`, `dir`, `breakpoint`, `exit` — as **bare calls**, and through the module that defines them (`builtins.eval`). A method that merely shares one of those names is not the builtin and is allowed: `re.compile(r'\d+')` and `model.eval()` were refused until the rule learned to ask *whose* `compile` this is. Neither could ever have been the builtin — the proxy refuses the real `compile` / `eval` / `open` by identity under any name, and `builtins` is on the denied-attribute list below, so a Tier-0 script has no route to it at all. **One exception, deliberate:** `.compile` is allowed *only* on `re`. `torch.compile(m)` — and `m.compile()` on an `nn.Module`, which PyTorch ≥ 2.2 added as a method — stay refused, because they hand the graph to TorchInductor, which generates C++/Triton and invokes a real compiler. That is the same door closed by refusing `torch.utils.cpp_extension`. Installed plugins are unaffected and may call both.
...
* **The name of a module you may not import, used as an attribute**: `torch.os`, `torch.sys`, `torch.serialization.pickle`, `json.codecs.sys`, `numpy.f2py.subprocess`. Libraries import things, so an allowlist that reads only `import` statements hands the blocked module straight over the moment you ask an allowed one for it by name. The names come from the same blocklist the import rule uses, minus `torch.signal` (torch's own DSP namespace, not the stdlib module), plus `builtins` — which cannot go on the shared blocklist, because that set doubles as the proxy's *"which module defines this value"* rule and `builtins` defines every `int`, `str` and `float` a library hands back, `math.pi` included. The proxy covers this case structurally; the name rule stays as the version the editor can show you while you type.
...
* Doors *inside* the allowed libraries that lead back out to the filesystem, the network, a compiler or another process: `torch.hub` (downloads and executes a remote `hubconf.py`), `torch.utils.cpp_extension` (compiles and runs C++), `torch.distributed`, `torch.multiprocessing`, `numpy.savetxt` / `loadtxt` / `fromfile` / `fromregex` / `tofile` / `save` / `dump` / `memmap`, `numpy.ctypeslib`, and the rest of `TIER0_DENIED_ATTRS` in `script_policy.py`. `dump` is the one on that list that is a method on the **array** rather than on the module: `numpy.zeros(3).dump(path)` pickles an array straight to any path you name, every array and scalar type carries it, and the content is substantially yours to choose. Because these rules are keyed on the attribute name **whatever the receiver**, the single entry closes every spelling of it at once — at the cost of `json.dump`, which goes with it (`json.dumps` is untouched, and `json.dump` needs a file object a script cannot obtain anyway). Importing one of those by name, or reaching it with a literal `getattr`, is refused the same way — as is `os.system` / `.popen` / `.spawnv` *as an attribute*, not only as a call, because `f = obj.system` then `f(cmd)` is one assignment away from any call-shaped rule.
...
**A green badge is not a guarantee.** The editor's check is the AST gate, so a script the gate has nothing to say about can still be refused mid-run — `numpy.f2py.crackfortran.myeval` and `torch.utils.collect_env.run` are numpy's and torch's own subpackages reached through an allowed module, and nothing in either line is spelled like a rule, while the proxy refuses both outright. When that happens you get the same policy message, with your line number, in the Execution Log.
...
* A [plugin pack](./plugins.md) — installable, versioned, and running under its own [three-tier policy](./plugins.md#security--three-tiers): pure computation needs no declaration, `network` / `filesystem` / `process-env` are declared in the manifest and confirmed by the user at install time, and anything beyond those needs `--trust-author`.
...
* **Capabilities that are not modules are refused by type.** A class is not a module, so the module rule above never saw `pathlib.Path` — and `Path` is arbitrary file read *and write*. It was reachable as `numpy.f2py.crackfortran.Path`, `numpy.f2py.f2py2e.rules.Path`, `torch.fx.graph_module.Path` and `torch.package.package_exporter.Path`, and as an *instance* at `numpy.testing.NUMPY_ROOT`: one class, five spellings, none of them a name any list would have held. A second rule now asks the same question of values that the first asks of modules — **what is this thing** — and refuses a value that is (or is an instance of) a known file-IO type (`pathlib.PurePath`, `io.IOBase`, `mmap.mmap`, `os.DirEntry`, including subclasses), or that is *defined by* a module this policy blocks (`subprocess.Popen`, `threading.RLock`, `importlib.find_spec`, ... whatever library re-exports them).
* **Subpackages are allowlisted individually, not by root.** "Any module whose top-level package is allowed" was the rule until it was measured: numpy and torch each ship subpackages that are tooling rather than numerics, and every one of these was a one-liner — `torch.utils.collect_env.run("cmd /c whoami")` (torch's own `subprocess` wrapper, `shell=True`), `numpy.f2py.crackfortran.myeval(...)` (a literal `eval()`), `torch.package` (writes source and then executes it), `numpy.lib.format.open_memmap(p, mode="w+")` (creates a file at any path), `torch.fx.symbolic_trace(m).to_folder(p)` (writes real `.py`), `torch.serialization.mkdtemp()`. None is a capability *type*, none is defined by a blocked module — they are numpy's and torch's own functions doing exactly what those subpackages exist for, which is why the type rules could not see them. The allowlist is now a list of **dotted module names** (`TIER0_MODULE_PATHS`), so a subpackage is reachable only if it is on it.
* **The residual: the type list is finite, and so is the module list.** The capability rule covers the file-IO types, anything a blocked module (or its C implementation — `nt`, `posix`, `_io`, `_thread`, ...) defines, and the builtins this policy removed from the namespace, refused by object identity so `os.getcwd` or `open` re-exported under a harmless name is the same function and the same answer. It does **not** cover a capability whose type is defined by an allowlisted library, nor the return value of a call, since the proxy hands out attributes and what a function does with what it returns is beyond it. **Two sweeps of the reachable surface**, both standing checks in the test suite. The first walks every attribute the policy allows, to depth 4 — 15 modules, 2,052 plain values — and finds no capability. The second inspects every callable it can reach for a parameter that looks like a file path. It originally stopped at module level (747 callables) and found one real hit, `numpy.fromregex`, which opens the path it is given. Stopping there was itself the defect: it never descended into the **classes** those modules hand over, and that is exactly where `numpy.ndarray.dump(path)` lives — pickle an array straight to any path, carried by every array and scalar type, reachable as roughly fifty spellings, and green through the editor's check the whole time it was open. The sweep now walks class methods too (15,851 of them across 462 classes). Both hits are closed, and the extended sweep is the one that runs on every commit. Running that same sweep with the type rules switched off also finds none, which is the honest reading: since the subpackage allowlist landed, those rules are a second lock with nothing currently behind them rather than the thing doing the work. Before it, the same sweep spanned 330 modules and the type rules were catching seven live capabilities. Both numbers describe today's libraries, not a property of the rules.
