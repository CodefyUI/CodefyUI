...
* **The residual: the type list is finite.** The capability rule covers the file-IO types and anything a blocked module defines. It does **not** cover a capability whose type is defined by an allowlisted library, nor a bare C function like `os.getcwd` bound under some harmless name, nor the return value of a call — the proxy hands out attributes, and what a function does with what it returns is beyond it. A sweep of 9,478 plain values reachable across 330 modules finds none today; with the rule disabled the same sweep finds seven, which is the honest measure of what it is doing and of how easily this surface changes. That is a statement about today's libraries, not a property of the rule.
