...
* **The residual: the type list is finite.** The capability rule covers the file-IO types, anything a blocked module (or its C implementation — `nt`, `posix`, `_io`, `_thread`, ...) defines, and the builtins this policy removed from the namespace, refused by object identity so `os.getcwd` or `open` re-exported under a harmless name is the same function and the same answer. It does **not** cover a capability whose type is defined by an allowlisted library — numpy's own `testing.tempdir` is one, closed by name in the denied-attribute list, which is a patch on that residual and not a fix for it — nor the return value of a call, since the proxy hands out attributes and what a function does with what it returns is beyond it. A sweep of 9,475 plain values reachable across 330 modules finds no capability today; with the rule disabled the same sweep finds seven, which is the honest measure of what it is doing and of how easily this surface changes. That is a statement about today's libraries, not a property of the rule.
