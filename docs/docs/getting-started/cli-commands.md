...
| `cdui start` | Production mode — single uvicorn on `:8000`, in the background (no Node needed). `--foreground` / `-f` runs it attached. It exits with code `1` when `frontend/dist/index.html` is missing; run `cdui install` or `cdui build`. When the dist was built from a commit whose frontend differs from the checkout's (recorded in `frontend/dist/build-info.json`), or uncommitted changes make `frontend/src` newer than the dist, it prints a warning and starts anyway. The warning says to run `cdui build` when pnpm is on `PATH`, otherwise `cdui update`. |
