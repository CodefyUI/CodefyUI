...
The release tarball redistributes compiled third-party code inside the prebuilt frontend bundle (React, React Flow, KaTeX and its fonts, CodeMirror, and others), all under MIT, ISC or BSD-3-Clause. The Python dependencies are not redistributed; they are installed from PyPI at install time. Two of them, pulled in transitively, use MPL-2.0, a file-level copyleft license: certifi (MPL-2.0) and tqdm (MPL-2.0 and MIT). CodefyUI uses both unmodified. [THIRD_PARTY_NOTICES.md](https://github.com/CodefyUI/CodefyUI/blob/main/THIRD_PARTY_NOTICES.md) collects the notices for the bundled code and lists the Python dependencies with their licenses; it ships inside the release tarball alongside `LICENSE`, `NOTICE`, `COMMERCIAL-LICENSE.md` and `CONTRIBUTING.md`.
