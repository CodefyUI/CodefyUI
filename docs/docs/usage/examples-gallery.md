...
- The four dataset trainers fetch their data on the first run and are offline after that: **Train CNN on MNIST** and **Train a Transformer on MNIST** download MNIST, **Train ResNet on CIFAR10** and the **ResNet-18 CIFAR-10 baseline** download CIFAR-10 (about 170 MB). Both land under `backend/data/`.
...
- Six examples under **Plugin Packs** fetch something on their first run, and each says so in the note on its canvas: the two MNIST trainers in the `foundations` and `deep` packs download MNIST into `backend/data/`, and the four `deep` pack examples that tokenise real text — **Self-Attention 101**, **Multi-Head Causal Attention**, **C4-2 Self-attention on real text** and **C4-4 LLM inference pipeline** — download the `cl100k_base` BPE table into the cache, which every later run reads locally. Everything else those four packs ship runs offline from the first click.
