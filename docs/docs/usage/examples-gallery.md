...
- Six examples under **Plugin Packs** fetch something on their first run, and say "needs a download" on the card: the two MNIST trainers in the `foundations` and `deep` packs download MNIST into `backend/data/`, and the four `deep` pack examples that tokenise real text — **Self-Attention 101**, **Multi-Head Causal Attention**, **C4-2 Self-attention over a tokenised sentence** and **C4-4 LLM inference** — download the `cl100k_base` BPE table into the cache, which every later run reads locally. Everything else those four packs ship runs offline from the first click.
