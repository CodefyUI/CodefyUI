...
- **Move around.** Scroll to zoom, down to 10%. The buttons at the bottom left zoom in, zoom out, fit the graph to the window, and lock the canvas so that nodes cannot be moved, connected or selected. Drag or scroll the minimap at the bottom right to move the view. Each tab keeps its own pan and zoom while the page is open; after a reload the view is fitted to the graph again.
...
- **Execution Log** lists what the current tab's run reports, one time-stamped line per event: the start and end of the run, each node's outcome (a node that is running or served from the cache adds no line), text from `Print` nodes, and images, charts and video clips, which are drawn in the log. Each **Run** starts with an empty log. The number on the tab counts the lines.
...
Messages appear at the bottom right, each with a close button. Success, information and warning messages close after four seconds. Errors, and messages with a button such as **Open Package Center** or **Reload**, stay until you close them.
...
- "This run was not started — …": **Run** was pressed while the server was still running this tab's previous run. Wait for it to finish, or press **Stop**.
