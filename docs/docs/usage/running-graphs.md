...
Some nodes still opt out of caching entirely with `cacheable = False`, for four different reasons:
...
`TrainingLoop` is in this group too, for the same reason in a different medium: what it produces is not the `model` handle it returns but the *change* to the weights that handle points at, plus a metric series per epoch. Neither survives a cache hit — so it re-executes every run, regardless of whether everything feeding it was unchanged. `EvaluateModel` is here for the narrower half of that: the accuracy is in its outputs, but the point it writes to the accuracy chart is not, so a hit left the chart empty on a run reported as successful.

**The node hands back a live, mutable object.** A `model`, an `optimizer` and a learning-rate scheduler are references to things training changes in place. The cache key describes how such an object was *built* — architecture, hyperparameters, upstream keys — and stops describing it the moment the first batch runs, so replaying the recorded handle hands the next run the object the previous run already trained. `ModelLoader`, `CheckpointLoader`, `Inference`, `Optimizer`, `LRScheduler`, `DQN`, `PPO` and `RewardModel` therefore all re-execute every run.

`ModelLoader` and `CheckpointLoader` sit in that group despite having a perfectly good content fingerprint, because the fingerprint describes what they *read* and not what they *write*: `load_state_dict` writes into the model you wired in, and a cache hit skips exactly that. It costs less than it sounds — opting out propagates downstream (see below), and whenever their `model` input is connected it comes from a weight-owning node, which is already non-cacheable, so on the usual wiring they were never being served from cache to begin with.
