...
The **Source Control** tab -- the branch icon in the sidebar rail -- is git for the project directory the server was started on. It shows what has changed since the last commit, stages and commits it, creates and switches branches, adds a remote and pushes to it, settles a merge conflict, lists the history, and opens any file's changes as a diff. A graph is a pair of plain JSON files on disk, so all of this is ordinary git against ordinary files: everything the tab does you can also do at a command line in the same directory, and everything you do at that command line shows up in the file lists, on the branch line and in any open **Branches**, **Remotes** or **Stashes** list within fifteen seconds while the tab is open. History is the exception: it is not polled (see below).

Two things have to be true before the tab can work. The server must have been started on a **project directory** -- `cdui project init my-project`, then `cdui start --project my-project`; a server started without one says "Source control needs a project directory." and prints those two commands, because the project is an argument to the server and no button in the browser can supply it. See [Project directories](./project-directories) for what a project is. And **git 2.23 or newer** must be installed on the computer that runs the server, not on the computer with the browser: without it the tab says "git is not installed on the server computer.", and with an older one `git {version} is too old; 2.23 or newer is required.` A project directory that is not a repository yet gets one button, **Initialize Repository**. It runs `git init` and writes the same `.gitignore` (first line `.env`) and `.gitattributes` that `cdui project init` writes. An existing `.gitignore` that does not ignore `.env` gets that one line appended. An existing `.gitattributes` is left as it is, so one written before 2.6.0 still lacks `*.json text eol=lf`; add that line yourself (see [Project directories](./project-directories#1-create-the-project) for what the file does). When the directory sits inside another repository, the screen says so first: `It sits inside another repository ({path}); initializing creates a separate one here.`

The server runs git as a subprocess, as its own operating-system user, and never interactively. Prompting is turned off outright -- `GIT_TERMINAL_PROMPT=0`, an emptied `GIT_ASKPASS`, and `ssh -oBatchMode=yes` unless you already chose how ssh runs (`GIT_SSH_COMMAND`, `GIT_SSH`, or `core.sshCommand` in your global config or this repository's) -- so a git that decides to ask for a password fails in a second instead of hanging until you close the tab, and the panel reports it as "The server computer has no saved credentials for this remote." CodefyUI stores no tokens, passwords or keys of its own. Whatever credential helper and SSH keys that user already has are what every fetch and push uses, which is why authentication is set up once, in a terminal on that machine, and never in the app.

## Set up git and GitHub

Do this once per computer that runs a server.

**Install git.** On Windows, `winget install Git.Git` -- Git for Windows bundles Git Credential Manager, which is the piece that remembers an HTTPS login. On macOS, `xcode-select --install` for Apple's git, or `brew install git` for a current one. On Ubuntu and Debian, `sudo apt install git`. The server looks for git about every 30 seconds, but only on the PATH it was started with. When the installer adds a new PATH entry, as Git for Windows does, restart the server as the screen says ("Install it, then restart the server."): `cdui stop`, then `cdui start --project <dir>` from a new terminal window, which has the new PATH. Restart after upgrading git as well: once the server has found git, it keeps that path and version until it restarts.
...
The tab can do the same thing once the project is a repository (press **Initialize Repository** first): **More actions** > **Commit identity...** opens a small form that shows what git currently answers with and whether it comes "from global git config" or "for this project". Saving from that form writes into this repository's own config, so it is the right place for a project that needs a different address from your usual one, and the wrong place for the machine-wide default -- for that, run the two commands above.
...
**Connect a repository.** Create an empty repository on GitHub -- no README, no `.gitignore`, no licence, so that its history is empty and your first push is not a merge. Then, in the tab, open **Remotes** and press the **+** (**Add Remote...**) on its heading: the name is `origin` unless you have a reason, and the URL is the one GitHub shows (`https://github.com/owner/repo.git`, or the SSH form). Once the branch has at least one commit, the header's second row offers **Publish Branch**, which pushes the current branch and sets it to track the remote one; with several remotes it opens a list of them to pick from. Before the first commit the branch line reads "No commits yet" and **Publish Branch** in **More actions** is greyed out with that reason, so make the first commit, then publish. From then on that row is a **Sync (pull, then push)** button instead.

## The daily loop

Save the graph (`Ctrl/Cmd` + `S`). Both files it writes -- `graphs/<name>.graph.json` and `layout/<name>.layout.json` -- appear under **Changes** at once rather than at the next poll. Stage what belongs in the commit with **Stage** on a row or **Stage All** on the group heading, write a message, and press **Commit** or `Ctrl/Cmd` + `Enter`. If you would rather not stage first, **Commit All (includes new files)**, behind the chevron beside the button, stages every change, new files included, and commits in one step. **Amend Last Commit** beside it is a switch: tick it and an **Amending** chip appears beside **Commit**, and the next Commit or Commit All replaces the last commit instead of adding one; with nothing staged it rewrites only the message. It switches itself off after that commit. It is greyed out while the branch has no commit, and reads "Cannot amend: the last commit is already pushed" once that commit is on the upstream.
...
New and modified files share the **Changes** list. **Unstage** on a row under **Staged Changes**, or **Unstage All** on its heading, takes files back out of the next commit. **Discard Changes** on a row under **Changes** asks `Discard changes to {name}?` and restores the file from the index, so a file that is also staged keeps its staged version; on a new file it asks `Delete {name}? It is not tracked by git and cannot be recovered.` and deletes it. **Discard All Changes** on the **Changes** heading asks `Discard {changed} changed file(s) and delete {untracked} untracked file(s)? This cannot be undone.` and does both for every file. Files git ignores, such as `.env`, a virtual environment or model weights, are never touched. Stage All, Unstage All and Discard All skip any file whose path goes through a linked folder (a symbolic link or a junction), and say `{count} file(s) skipped (they point outside the project)`. Discard All still deletes an untracked link itself, never the files it points to. Stage All also stages conflicted files, which marks them resolved as they are on disk, conflict markers included; during a merge, settle each file with the **Merge Changes** rows described under Conflicts.
...
**Hide layout files** in **More actions** keeps `layout/*.layout.json` rows out of the **Changes** list, since every save writes one. While it is on, `({count} hidden)` follows the item's name, and the choice is remembered in this browser. **Staged Changes**, Stage All and Discard All still include those files. **Setup guide**, the last item in the menu, opens this page.
...
**Remotes** lists each remote with the URL git fetches from, plus **Change URL** and **Remove**; **Add Remote...** is the **+** on the heading. A URL that contains a credential is shown with it masked (`https://alice:***@host/r.git`), and **Change URL** opens an empty prompt, so the masked text is never written back. Only `https://`, `ssh://`, `file://` and the `git@host:owner/repo.git` form are accepted, and the server allows git exactly those three protocols -- the transports whose "URL" is a command line to run cannot be configured at all.
...
When **Pull** or **Sync** is refused with "Local and remote branches have diverged.", the error line carries a **Merge remote changes** button beside Dismiss; it pulls with a merge, which is the step the fast-forward pull declined to take. A plain **Push** to a remote that has commits you do not have is refused with "The remote has commits you do not have. Pull first." and no button: run **Sync** or **Pull** first. A refusal that carries git's own output has a **Details** disclosure with the raw tail in it, redacted of anything that looked like a credential.
...
API-key fields on LLM nodes are secret parameters: a key typed into one is blanked from the saved graph, so it never reaches a commit, and it is gone after a reload. [Version control your graphs](./version-control-graphs#secrets-keep-keys-out-of-your-graphs) has the environment variables to use instead.
...
- **History pages by offset.** Thirty commits at a time, so a commit made between two pages can shift the window; **Refresh** re-reads the pages you have loaded, from the first, up to 100 commits, and **Load more** brings back the rest.
...
| "Local and remote branches have diverged." | Both sides have commits the other does not; the fast-forward that Pull and Sync attempt is impossible. | Press **Merge remote changes** beside the message, settle any conflicts, then push. |
| "The remote has commits you do not have. Pull first." | A push found commits on the remote that this branch does not have. | Press **Sync**, or **Pull** and then **Push**. |
...
| "Your local git configuration refused this push. Check push.default and the upstream branch name." | git's own `push.default` rules, not the remote's. | Read git's sentence under **Details**; usually `git config --global push.default current` or a matching upstream branch name fixes it. `git branch -vv` shows the upstream. |
| `git {version} is too old; 2.23 or newer is required.` | The server computer's git predates `git restore` and `git switch`, which the tab uses. `?` in place of the version means `git --version` could not be read. | Upgrade git there, then restart the server: it reads the version once per process. |
