...
A graceful shutdown — Ctrl+C on `cdui start --foreground`, or `systemctl stop` on the unit in [Deployment](./deployment#a-systemd-unit) — immediately retires every waiting run as `interrupted`, writes its normal stop event, and gives executing runs 5 seconds to stop cooperatively. `cdui stop` does not wait for that: on Windows it force-kills the server's process tree (`taskkill /F /T`), and on Linux and macOS it sends SIGTERM and then SIGKILL about 2 seconds later. `cdui stop`, any other hard kill, or an executing run that outlasts the 5 seconds can leave rows as `queued` or `running`. At the next startup, recovery changes both statuses to `interrupted` without writing a stop event; neither resumes.
