...
一個 run 屬於**伺服器**，不屬於送出它的人。關掉分頁、關掉 terminal、登出都沒關係 — run 會繼續跑，之後可以透過 API 或 CLI 回來看它。
...
`GET /api/runs` 會為每個排隊中的 run 回報 `queue_position` — 從 1 開始，而且是**在它自己那條裝置佇列裡**的位置，所以排在兩個 CPU run 後面的 CPU run 就是第 3 位，不管它前面送出過幾個 CUDA run。`cdui run --wait` 等待時會印出這個位置；未來的 Runs 面板也會直接顯示它。
...
一個 run 在它的圖跑完的當下就把裝置名額還回去，此時它還沒寫完最終狀態與結束事件。下一個 run 因此會在前一個還在收尾時就啟動，所以剛好落在這個空檔的查詢，可能會在上限為 1 的佇列上看到兩筆 `running`。沒有任何東西被執行兩次 — 裝置確實是閒置的 — 而且這是刻意的：另一個選擇是讓 GPU 空等一次資料庫寫入，更糟的是告訴畫布「跑完了」卻又拒絕下一次點擊。
:::
