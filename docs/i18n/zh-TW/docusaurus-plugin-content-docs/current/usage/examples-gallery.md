...
| **其他** | 沒有宣告區塊的範例 — 例如沒有附上範例集資訊的第三方套件包。 |
...
`section` 只能是 `quickstart`、`training`、`llm`、`concepts` 與 `architectures` 其中之一。`family` 是**模型架構**區塊裡的小標題（上面那五個家族之一），其他地方都不會讀它。`order` 在同一區塊（模型架構則是同一個家族）內由小到大排序，沒有寫 `order` 的範例排在有寫的後面。清單讀不懂的欄位 — 不認識的 `section`、不是整數的 `order` — 只有那一個欄位會變成空值，範例會落到**其他**區塊，而不是讓整個範例集掛掉。外掛提供的範例不管宣告什麼，都會列在它自己的套件包底下。
