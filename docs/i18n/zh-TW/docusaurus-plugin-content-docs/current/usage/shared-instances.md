...
型別標示為 SECRET 的參數，例如 `LLMChat` 節點的 `openai_api_key`，屬於輸入該值的人，處理方式不同。伺服器會從寫出的每份副本中清除這些值，包括已儲存的 graph、匯出檔案、已發佈應用程式版本、預設組合（preset）、產生的 Python 程式碼與執行紀錄。按**執行**時，你輸入的值會隨該次執行傳到這台伺服器，`LLMChat` 節點會優先使用它，而不是實例環境變數中的金鑰。伺服器不認得某個節點的型別時（例如已停用的自訂節點、已移除的外掛），無法判斷該節點的哪些參數屬於 SECRET，因此會在執行紀錄中清除該節點的所有參數值。
...
- 每個實例使用自己的安裝目錄（安裝程式的 `CODEFYUI_DIR`）、環境變數檔與埠號。兩個伺服器不會共用同一個使用者資料目錄或資料庫：第二個伺服器會拒絕啟動，記錄一行訊息並以非零離開碼結束。因此，從同一個安裝啟動第二個實例時，必須另外設定 `CODEFYUI_USER_DATA_DIR`（工作階段權杖、ChatGPT 登入、下載的外掛與其 lockfile、下載快取、套件包的控制檔）與 `CODEFYUI_DB_PATH`（執行紀錄、已發佈應用程式、API key）；即使如此，它仍會共用已儲存的 graph、模型、圖片、媒體與上傳的資料檔，以及套件包與外掛安裝時加入套件的 Python 環境。`cdui start` 每個安裝只會執行一個背景伺服器，而 `cdui stop` 會停止從該安裝啟動的所有伺服器。
