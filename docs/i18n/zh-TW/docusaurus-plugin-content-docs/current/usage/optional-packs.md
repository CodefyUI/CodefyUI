...
有兩種安裝沒辦法在伺服器還跑著的時候做完，因為它們要換掉的，正是那台伺服器已經載入的東西。**GPU PyTorch** 永遠屬於這一類：把 CUDA 或 ROCm 的 wheel 從「正在處理這個請求的那個直譯器」腳下抽換掉，是任何行程都不可能對自己做的事。另一類是任何在線上安裝時撞上[疑難排解](#疑難排解)裡那個相依衝突的套件包 —— constraints 檔會讓它直接停下來，而不是換到一半，而能把它裝完的正是一次重新啟動。這兩種情況，只要伺服器是用 `cdui start` 啟動的，它就能在「自己不存在的那段空檔」裡把安裝做完：先把要裝什麼寫下來，啟動一個活得比自己久的輔助程式，然後把自己關掉。輔助程式會等這個行程真的消失，執行安裝，把結果記下來，再用這次 `cdui start` 收到的完全相同的參數把伺服器啟動回來 —— 所以瀏覽器上原本開著的那個網址，回來之後還是通的。輔助程式跑的這次安裝，刻意不套用線上安裝那份把每個套件都釘住的 constraints 檔，而這正是把它挪到這裡做的用意：若套件包需要的 torch 和這個 venv 裡的不同，它可以把 torch 換成那些套件要求的版本，而不是停在衝突上。

**面板會做的事。** GPU PyTorch 那張卡片會讓你選一個版本，附上「安裝完成後伺服器會重新啟動；有圖在執行時會拒絕開始。」這句提醒，以及一個**安裝並重新啟動**按鈕；終端機的指令仍然印在下面，當作「你也可以自己來」的替代做法。按下按鈕會先跳出確認（「要安裝 cu128 並重新啟動伺服器嗎？」），接著整個頁面會被一層**伺服器重新啟動中**的遮罩蓋住，上面顯示已等待的秒數 —— 因為伺服器正在退場的那段時間裡，程式裡其他每一個畫面都是騙人的。等到回答的已經是「另一個」伺服器行程，頁面會自己重新載入，而重新載入後的頁面會用一則提示回報結果：「伺服器已重新啟動，GPU PyTorch 可以使用了。」，或是「伺服器已重新啟動，但安裝 GPU PyTorch 失敗：」加上原因；若連原因都沒有，就改附上安裝程式的最後輸出。線上安裝因相依衝突而停下來時，活動面板的橫幅也會提供同一件事：指令區塊旁邊多一個**重新啟動伺服器並安裝**按鈕。重新啟動安裝只會裝套件包的 **Python 套件**或那個 torch wheel，**絕不會裝模型項目** —— 輔助程式所在的那個直譯器裡，完全沒有這個程式的下載器 —— 所以若這個套件包還有模型檔，之後要再做一次一般的安裝才會下載，確認視窗上寫的就是這件事。

**什麼時候會提供，什麼時候會被拒絕。** 提不提供是伺服器決定的，不是瀏覽器：`GET /api/packs` 會帶上 `restart_available`，只有它為真時才會有那個按鈕。三個條件必須同時成立。伺服器必須是用 `cdui start` 啟動的，因為只有它知道怎麼再啟動一次 —— `cdui dev` 是原地重載，沒有任何人能把它重新拉起來，所以卡片會直說，並改給你指令。啟動器也必須還在磁碟上：伺服器啟動之後被搬走或刪掉的 checkout，沒辦法把任何東西帶回來。而且 `CODEFYUI_ENABLE_RESTART_INSTALL` 不能設成 `0` —— 那是給「重新啟動在這台機器上回不來」準備的總開關：設下去之後，這台機器上的每一次重新啟動安裝都會改成告訴你該執行哪一行指令，而且不需要降級任何東西。就算三個條件都成立，在下列情況下請求仍然會被拒絕，而且什麼都不會寫下、什麼都不會安裝：有圖正在執行或排隊等待（重新啟動會把那次執行一起帶走）、已經有另一個安裝在跑，或者已經有另一個重新啟動安裝在等著。還有兩件事值得在按下去之前知道：輔助程式會檢查 venv 所在磁碟區的可用空間 —— torch wheel 要 3 GB，套件包的 Python 套件要 1 GB —— 空間不夠時它會記下一個失敗的工作，而不是裝到一半；另外，伺服器一定會以背景常駐的方式回來，就算原本是用 `cdui start -f` 在前景啟動的也一樣，因為輔助程式沒有主控台可以交還給它。

**檔案在哪裡。** 有三個檔案負責把一次重新啟動送過那段空檔，它們都在使用者資料根目錄底下 —— Windows 是 `%LOCALAPPDATA%\codefyui`，macOS 是 `~/Library/Application Support/codefyui`，Linux 是 `~/.local/share/codefyui`；有設 `CODEFYUI_USER_DATA_DIR` 時則是 `<dir>`。pending 檔就是那張「認領單」，上面寫著正在做事的那個輔助程式。只要那個輔助程式還活著 —— 或在它把自己的行程編號寫進認領單之前，這個檔案還不到六十秒 —— 這次重新啟動就還在**收尾中**（finishing）：`cdui start` 會說有一次重新啟動安裝正在收尾，不在那個輔助程式正在改寫的 venv 上再起第二台伺服器，並要你去看 `cdui status`；而只要寫下認領單的那台伺服器還活著、認領單也還不到十五分鐘，套件中心就會拒絕另一個重新啟動安裝。等到輔助程式不在了 —— 或它根本沒出現，而那六十秒已經過去 —— 這張認領單就算**已中斷**（abandoned）：`cdui start` 會刪掉它並照常啟動，伺服器自己啟動時也會清掉它，而新的重新啟動安裝則直接覆蓋它。每一個使用者資料根目錄只有一張認領單，而它是為「一台伺服器」寫的：在同一個根目錄下同時跑兩台受管理的伺服器 —— 前景的 `cdui start -f` 和常駐的 `cdui start` 並存 —— 是不支援的做法，這種安排下的重新啟動安裝，也不是這個設計能做對的事。請給第二台自己的 `CODEFYUI_USER_DATA_DIR`。

```text
<user data>/packs/pending_restart.json      這次重新啟動要安裝什麼
<user data>/packs/last_restart_job.json     結果如何；重新載入後的頁面會讀它，`cdui status` 一小時內也會
<user data>/packs/logs/restart-<job>.log    安裝程式印出的所有東西
```

**如果伺服器沒有回來。** 遮罩會用兩種方式放棄等待。若過了三十秒，伺服器連「停止回應」都沒發生過，那就是根本沒有人接手這次重新啟動，它會顯示「伺服器沒有重新啟動。請執行下面的指令後重新載入：」。若過了十分鐘，它會顯示「等了 10 分鐘，伺服器還沒恢復。」兩種情況都會附上指令（伺服器有給的話）和一個**立即重新載入**按鈕；而且不再盯著這層遮罩，也不會讓這次安裝的任何資訊消失：輔助程式不管有沒有人在看，都會把結果記下來。接下來該看的是 `cdui status`，它會分辨上面那兩種認領單狀態：中文介面下的「重啟安裝」那一行會指出是哪個套件包，輔助程式還在做事時顯示「收尾中」，不在了則顯示「已中斷」；另外工作結束後的一小時內，會多出一行「上次重啟安裝」，帶著結果紀錄裡的訊息。重新啟動本身失敗時，並不會蓋掉安裝本身的結果：紀錄會保留安裝自己的狀態，加上 `relaunch: failed`，並把記錄檔路徑接在訊息後面 —— 這足以讓 `cdui status` 把那一行「上次重啟安裝」標成失敗，就算套件本身其實裝得好好的。安裝程式的完整輸出在 `packs/logs/restart-<job>.log`。等認領單顯示「已中斷」之後，用 `cdui start` 就能手動把伺服器叫回來，而且它會順手清掉那張認領單；還在「收尾中」時，`cdui start` 會直接婉拒，並要你去看 `cdui status`。
...
- **安裝到一半伺服器就不見了，而且沒有回來。** 只有重新啟動安裝會刻意把伺服器停掉 —— GPU PyTorch，或是線上安裝撞到上面那個衝突的套件包 —— 而且不論安裝成功與否，負責的輔助程式都會把伺服器重新啟動回來；所以伺服器如果還是不見，代表輔助程式也死了，或是重新啟動本身失敗了（這種情況下，結果紀錄會保留安裝自己的狀態，加上 `relaunch: failed`，並把記錄檔路徑接在訊息後面，所以 `cdui status` 會把它顯示成失敗）。請先執行 `cdui status`。中文介面下的「重啟安裝」那一行顯示「收尾中」，表示輔助程式還在做事，該做的事就是等：這時 `cdui start` 會婉拒，說有一次重新啟動安裝正在收尾並要你去看 `cdui status`，而不是在別人正在改寫的 venv 上再起一台伺服器。等它顯示「已中斷」，或是「上次重啟安裝」那一行已經出現，就去讀 `<user data>/packs/logs/restart-<job>.log`，看安裝程式到底印了什麼，再執行 `cdui start` —— 它會在啟動時刪掉殘留的認領單，所以下一次不會再被面板用「已經有一個重新啟動在進行中，請等伺服器回來。」擋下來。面板只有在它自己的遮罩還在等、而且結果紀錄寫的正是它在等的那次安裝時才會回報結果：那張記著「要回報哪個套件、哪一次安裝」的便條，在遮罩放棄等待的當下就被丟掉了，所以之後才手動重新載入的分頁 —— 或是從頭到尾沒在看的分頁 —— 什麼都不會說。答案改看 `cdui status`：安裝結束後的一小時內，都在那一行「上次重啟安裝」上，其餘的細節則在記錄檔裡。
