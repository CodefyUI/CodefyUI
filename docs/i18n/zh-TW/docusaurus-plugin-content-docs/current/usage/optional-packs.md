...
**如果伺服器沒有回來。** 遮罩會用兩種方式放棄等待。若過了三十秒，伺服器連「停止回應」都沒發生過，那就是根本沒有人接手這次重新啟動，它會顯示「伺服器沒有重新啟動。請執行下面的指令後重新載入：」。若過了十分鐘，它會顯示「等了 10 分鐘，伺服器還沒恢復。」兩種情況都會附上指令（伺服器有給的話）和一個**立即重新載入**按鈕；而且不再盯著這層遮罩，也不會讓這次安裝的任何資訊消失：輔助程式不管有沒有人在看，都會把結果記下來。接下來該看的是 `cdui status`，它會分辨上面那兩種認領單狀態：中文介面下的「重啟安裝」那一行會指出是哪個套件包，輔助程式還在做事時顯示「收尾中」，不在了則顯示「已中斷」；另外工作結束後的一小時內，會多出一行「上次重啟安裝」，帶著結果紀錄裡的訊息。重新啟動本身失敗時，也會被記成一個失敗的工作，並指出該讀哪一份記錄檔；安裝程式的完整輸出在 `packs/logs/restart-<job>.log`。等認領單顯示「已中斷」之後，用 `cdui start` 就能手動把伺服器叫回來，而且它會順手清掉那張認領單；還在「收尾中」時，`cdui start` 會直接婉拒，並要你去看 `cdui status`。
...
- **安裝到一半伺服器就不見了，而且沒有回來。** 只有重新啟動安裝會刻意把伺服器停掉 —— GPU PyTorch，或是線上安裝撞到上面那個衝突的套件包 —— 而且不論安裝成功與否，負責的輔助程式都會把伺服器重新啟動回來；所以伺服器如果還是不見，代表輔助程式也死了，或是重新啟動本身失敗了（這種情況會被記成一個失敗的工作，並指出該讀哪一份記錄檔）。請先執行 `cdui status`。中文介面下的「重啟安裝」那一行顯示「收尾中」，表示輔助程式還在做事，該做的事就是等：這時 `cdui start` 會婉拒，說有一次重新啟動安裝正在收尾並要你去看 `cdui status`，而不是在別人正在改寫的 venv 上再起一台伺服器。等它顯示「已中斷」，或是「上次重啟安裝」那一行已經出現，就去讀 `<user data>/packs/logs/restart-<job>.log`，看安裝程式到底印了什麼，再執行 `cdui start` —— 它會在啟動時刪掉殘留的認領單，所以下一次不會再被以「a restart-mode install is already pending」為由拒絕。等伺服器有回應之後重新載入瀏覽器分頁，它就會回報這次安裝的結果。
