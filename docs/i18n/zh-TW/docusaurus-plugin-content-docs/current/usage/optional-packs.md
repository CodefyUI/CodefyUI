...
**面板會做的事。** GPU PyTorch 那張卡片最上面是一行事實 —— 偵測到的 GPU，以及目前安裝的版本；只有在建議的版本和已經裝著的不一樣時，才會把建議接在同一行後面 —— 接著是一列控制項，裡面放著版本選單和一個**安裝並重新啟動**按鈕。按鈕底下只有一行說明：「有圖在執行時不會開始。」；伺服器會重新啟動這件事，按鈕自己的標籤和確認視窗的問句都已經說過了，所以這一行只說它們兩個都沒說的那一半。終端機的指令仍然在，收在**手動安裝指令**這個摺疊區塊裡：旁邊既然有一個會做同一件安裝的按鈕，指令就是一個選擇，而不是一道指示。而在卡片沒有按鈕的情況下 —— `cdui dev`，或伺服器回報自己無法重新啟動的任何其他原因 —— 指令就直接攤開來印，上面附一句說明原因的話。按下按鈕會先跳出確認（「要安裝 cu128 並重新啟動伺服器嗎？」），接著整個頁面會被一層**伺服器重新啟動中**的遮罩蓋住，上面顯示已等待的秒數 —— 因為伺服器正在退場的那段時間裡，程式裡其他每一個畫面都是騙人的。等到回答的已經是「另一個」伺服器行程，頁面會自己重新載入，而重新載入後的頁面會用一則提示回報結果：「伺服器已重新啟動，GPU PyTorch 可以使用了。」，或是「伺服器已重新啟動，但安裝 GPU PyTorch 失敗：」加上原因；若連原因都沒有，就改附上安裝程式的最後輸出。線上安裝因相依衝突而停下來時，活動面板的橫幅也會提供同一件事：指令區塊旁邊多一個**重新啟動伺服器並安裝**按鈕。重新啟動安裝只會裝套件包的 **Python 套件**或那個 torch wheel，**絕不會裝模型項目** —— 輔助程式所在的那個直譯器裡，完全沒有這個程式的下載器 —— 所以若這個套件包還有模型檔，之後要再做一次一般的安裝才會下載，確認視窗上寫的就是這件事。
