...
* **殘留風險：一般的值不會被代理。** 代理包的是模組。函式庫交回來的其他東西——張量、陣列、類別、函式，以及任何呼叫的回傳值——都是真正的物件；如果**它們**產出一個模組，那就只有關卡的名稱規則看得到。目前 numpy／torch／標準函式庫的屬性表面上沒有這種情況（把每個公開屬性往下走兩層，找不到任何一般的值身上掛著模組），但這一條確實是對「今天這些函式庫」的描述，而不是規則本身的性質，這裡也就照實寫出來。
