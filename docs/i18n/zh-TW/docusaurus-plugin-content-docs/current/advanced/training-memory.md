...
- MPS 除了 `fp32` 以外都會拿到 `fp32`。兩種 16 位元模式在 torch 2.11 可以執行，但較慢：在 M3 上量測，ResNet-18 在 CIFAR 上 fp32 為 17 ms/step、bf16 為 30 ms、fp16 為 37 ms，配置器記憶體沒有變化。MPS 的 kernel 是 float32 原生的，半精度只是在每個 op 前後增加轉換。
