...
2. **驗證**——DAG 檢查、連接埠/型別安全，以及一個必要的 [`Start`](/usage/first-graph) 節點。節點只要能透過 trigger 邊抵達，或是為某個能抵達的節點提供資料連接——不論該連接埠是必要還是選用皆然——（不論是直接提供還是透過一連串節點串接），就會執行——像 `Dataset` 或轉換鏈開頭這類自身沒有 trigger 的根節點，不會因此被剪掉，而是會被保留下來，因為下游有節點會用到它的輸出（core#201）。
