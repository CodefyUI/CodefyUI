...
## 使用預設模組 {/* #using-a-preset */}

預設模組會出現在側邊欄**節點**分頁中、位於節點分類下方的**預設組合**分類，也會出現在雙擊畫布所開啟的快速搜尋。該分頁的搜尋框同時會比對節點名稱與預設模組名稱。預設模組和分頁中的其他項目一樣，直接拖曳到畫布上即可。放置後，預設模組的行為與其他節點相同。它會暴露子圖內所有未連接的連接埠，以及每個內部節點的所有非 secret 參數。
