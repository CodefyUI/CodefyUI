...
1. 依上述步驟安裝外掛。
2. 點擊編輯器右下角的 **Graph Copilot** 按鈕（對話泡泡圖示），開啟面板。
...
- **Chat**：與代理的對話。
- **Lab**：從 Chat 執行的實驗研究，以及從檔案匯入的研究（**Import portable study**）。研究會在目前圖的記憶體複本上執行每個變體，並將結果排名。只有在你要求代理套用勝出的變體，而且它只改參數時，畫布才會改變：此時會以一個復原步驟套用；研究執行期間圖若有變動，就不會套用。
