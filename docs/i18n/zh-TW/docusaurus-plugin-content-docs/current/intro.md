...
- **儲存與搬移圖。** 側邊欄的**圖表**分頁會列出儲存在伺服器上的圖。**匯出 → 工作區 (.cduiworkspace)** 會把所有開啟的分頁存成一個檔案，可在另一個瀏覽器匯入。請參閱[分頁與持久化](/usage/tabs-persistence)。
- **版本控制與發佈。** **版本控制**分頁會在[專案目錄](/usage/project-directories)上執行 git；[發佈](/usage/publish)會把圖變成有版本、受 API key 保護的 HTTP 端點。
