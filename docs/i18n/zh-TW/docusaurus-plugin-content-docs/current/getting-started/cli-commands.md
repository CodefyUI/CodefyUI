...
| `cdui plugin update [<id>]` | 依記錄下來的 ref 重新讀取外掛自己的儲存庫，若 commit 有變就重新安裝；不指定 id 就更新所有已安裝的第三方外掛包。外掛包會保留它安裝時的型錄項目，所以官方外掛在第一次更新之後仍然讀作官方；如果你先前把它停用了，更新後它也仍然是停用的。若某個儲存庫的 manifest 現在宣告的是**另一個**外掛 id，這次更新會被拒絕（離開碼 `1`），不會用新名字裝進來——更新一個外掛絕不會裝上另一個，也不會蓋掉你已經有的那一個；要的話請把改名後的儲存庫當成新外掛安裝。內建外掛包會被跳過——它們用 `cdui update` 更新。 |
