...
| `cdui cache prune` | 經過 `[y/N]` 確認後刪掉那些項目。`--older-than DAYS` 會保留比這個天數更近寫入的項目（看的是最後**寫入**時間 —— 讀取不算，因為快取命中並不會動到檔案）。`--yes`／`-y` 可跳過確認，沒有終端可確認時必須加。若有從這個安裝啟動的伺服器正在執行則會拒絕：裡面的圖可能正讀到一半。 |

離開碼（給腳本用）：`0` 完成（包含「沒有東西需要刪除」）、`1` 在確認時選了不要或有項目刪不掉、`2` 還沒開始就拒絕（`--older-than` 是負數、沒有終端可確認）、`3` 伺服器執行中 —— 請先 `cdui stop`、`130` 以 `Ctrl+C` 中斷。
