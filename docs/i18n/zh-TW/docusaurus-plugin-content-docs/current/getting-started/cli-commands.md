...
| 指令 | 說明 |
|------|------|
| `cdui plugin install <name\|url>` | 安裝一個外掛包——型錄名稱、`owner/repo[@ref]`，或完整的 GitHub URL。型錄名稱同時涵蓋內建外掛包與住在各自儲存庫裡的官方外掛，所以 `cdui plugin install graph-copilot` 會替你去抓型錄指名的那個儲存庫。manifest 會先被讀出來並顯示給你——這個外掛包是什麼、會加哪些 Python 套件、它宣告了哪些能力——所以在下載任何東西之前你就會被詢問，而回答「不要」不用付出任何代價。`--force` 會蓋過已經裝好的版本，`-y` 只跳過 `Proceed?` 那一題，`--accept-capabilities` 會直接授予 manifest 宣告的能力而不詢問，`--trust-author` 則接受一個要求匯入白名單以外模組的外掛包。 |
...
| `cdui plugin update [<id>]` | 依記錄下來的 ref 重新讀取外掛自己的儲存庫，若 commit 有變就重新安裝；不指定 id 就更新所有已安裝的第三方外掛包。外掛包會保留它安裝時的型錄項目，所以官方外掛在第一次更新之後仍然讀作官方；如果你先前把它停用了，更新後它也仍然是停用的。若某個儲存庫的 manifest 現在宣告的是**另一個**外掛 id，這次更新會被拒絕（離開碼 `1`），不會用新名字裝進來——更新一個外掛絕不會裝上另一個，也不會蓋掉你已經有的那一個；要的話請把改名後的儲存庫當成新外掛安裝。內建與連結的外掛包都會被跳過——內建外掛包用 `cdui update` 更新，而連結的目錄本來就是作者磁碟上此刻的樣子。 |
| `cdui plugin list` | 列出已安裝的外掛包，以及還在等你決定的內建外掛包。 |
| `cdui plugin info <id>` | 顯示某個外掛包的 manifest、涵蓋的課程與節點名稱。 |
| `cdui plugin search <query>` | 查詢外掛型錄。 |
| `cdui plugin uninstall <id>` | 移除一個已安裝的外掛包。若是內建外掛包，這個移除會被記住，`cdui plugin sync` 不會再把它裝回來；要拿回它就執行 `cdui plugin install <id>`。不論哪一種，外掛包的 Python 套件都會留著——從「已經匯入它們的那個直譯器」裡面反安裝，正是讓伺服器只載入一半的方法——所以想要回那些空間，請在伺服器停掉後自行移除。 |

離開碼（給腳本用）：`0` 完成——包含在 `Proceed?` 那一題回答不要，那是一個答案而不是失敗；`1` 安裝失敗，或某項能力／模組要求被拒絕；`2` 還沒開始就拒絕（來源無法解析，或根本沒給來源）；`3` 伺服器執行中無法安裝這個外掛的 Python 套件 —— 會印出改用的指令；`130` 以 `Ctrl+C` 中斷。
...
有些節點會把「算得出來的結果」寫到磁碟，而在你動手刪掉之前，沒有任何東西會清掉它們。最大的一個是 `LMTokenizedDataset`：它把整份語料打包成一條 token 流放在 `<data>/cache/lm_blocks/`，語料、tokenizer、`seq_len`、`append_eos`、`max_tokens` 每換一種組合就多一個檔案，每個 token 佔 8 bytes —— 一億 token 的語料每個檔案約 800 MB，所以把 `seq_len` 掃過三個值，就留下三份完整的副本。這幾個指令只處理這類「重算得回來」的快取：下載回來的模型與素材不會被動到（那是 `cdui packs remove` 的事），執行結果、存下來的模型與圖檔也一樣。

| 指令 | 說明 |
|------|------|
| `cdui cache list` | 列出每一個衍生快取：有幾個項目、佔多少磁碟空間，以及放在哪個目錄。 |
| `cdui cache prune` | 經過 `[y/N]` 確認後刪掉那些項目。`--older-than DAYS` 會保留比這個天數更近寫入的項目（看的是最後**寫入**時間 —— 讀取不算，因為快取命中並不會動到檔案）。`--yes`／`-y` 可跳過確認，沒有終端可確認時必須加。若有**背景**伺服器（`cdui start`）正在執行則會拒絕：裡面的圖可能正讀到一半。只認得背景的 —— 前景的 `cdui dev` 或 `cdui start -f` 不會寫下行程編號，誰也偵測不到，請自己先把它停掉。 |

兩個指令都吃 `--project <dir>`，就是你啟動伺服器時用的那個旗標：專案模式的快取放在 `<dir>/assets/cache/`，不加這個旗標，它們回答的是這個安裝的 `<data>/cache`。

離開碼（給腳本用）：`0` 完成（包含「沒有東西需要刪除」）、`1` 在確認時選了不要或有項目刪不掉、`2` 還沒開始就拒絕（`--older-than` 是負數、沒有終端可確認）、`3` 背景伺服器執行中 —— 請先 `cdui stop`、`130` 以 `Ctrl+C` 中斷。
