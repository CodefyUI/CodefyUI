const en = {
  ...
  // Inline tensor grid editor (ConfigPanel). `{mode}` is rendered as a <code>
  // span holding the `value_mode` parameter name: it is the one word here the
  // reader has to go and find in the panel, so it must not read as prose.
  'tensorGrid.needExplicit': 'Set {mode} to explicit to edit these values.',
  'tensorGrid.tooLarge':
    '{total} elements is too many to edit here (max {max}) — set {mode} to random, zeros, ones or arange.',
  'tensorGrid.fillZero': 'Fill 0',
  'tensorGrid.fillOne': 'Fill 1',
  'tensorGrid.random': 'Random',
  'tensorGrid.cells': '{count} cells',
  // `dim` stays the tensor API's own word in every locale: it labels the axis
  ...
  'results.trainingConfig': 'Config',
  'results.trainingEmpty': 'No training data yet.',
  'results.clear': 'Clear',
  'results.empty': 'Run the pipeline to see output.',

  // Preset
  'preset.badge': 'PRESET',
  'preset.configure': 'Configure',
  // The button's own label is a bare verb, and the PRESET badge and node count
  // that tell a sighted reader what it configures are separate elements.
  'preset.configure.aria': 'Configure preset',
  ...
  'subgraph.detail.interface': 'Interface',
  ...
  'layersEditor.import.title': 'Import an architecture file',
  'layersEditor.export.title': 'Export as JSON',
  'layersEditor.empty': 'Drag a layer here to start',
  ...
  'layersEditor.import.noContent': 'No layers or SequentialModel nodes in this file.',
  ...
  'layersEditor.autoLayoutTitle': 'Arrange top-to-bottom by connections',
  ...
  'layersEditor.validation.inputNeedsPort': 'Input node must have at least one port',
  'layersEditor.validation.outputNeedsPort': 'Output node must have at least one port',
  'layersEditor.validation.dupInputPorts': 'Input port names must be unique',
  'layersEditor.validation.dupOutputPorts': 'Output port names must be unique',
  'layersEditor.validation.outputEdgeCount':
    "Output port '{port}' must have exactly 1 incoming edge (got {count})",
  'layersEditor.validation.inputPortUnused': "Input port '{port}' is unused",
  'layersEditor.validation.layerEdgeCount':
    "Layer '{layer}' must have exactly 1 incoming edge (got {count})",
  'layersEditor.validation.unreachable': "Node '{layer}' is not reachable from Input",
  'layersEditor.validation.cannotReachOutput': "Node '{layer}' cannot reach Output",
  'layersEditor.import.emptyGraph': 'Empty or invalid graph',
  'layersEditor.category.convolution': 'Convolution',
  'layersEditor.category.normalization': 'Normalization',
  'layersEditor.category.pooling': 'Pooling',
  'layersEditor.category.regularization': 'Regularization',
  'layersEditor.category.linear': 'Linear',
  'layersEditor.category.recurrent': 'Recurrent',
  'layersEditor.category.attention': 'Attention',
  'layersEditor.category.utility': 'Utility',
  'layersEditor.category.activation': 'Activation',
  ...
  'results.clickToExpand': 'Show error details',
  'results.clickToHighlight': 'Highlight this node',
  'results.imageAlt': 'Node output image',
  'results.videoAlt': 'Node output clip',
  ...
  'inspector.noTensors': 'No tensors',
  'inspector.tensor.module': 'Module',
  'inspector.tensor.params': 'params',
  'inspector.empty.notRun': 'Nothing captured yet',
  'inspector.empty.notRunHint': 'Turn on Record node outputs in Settings, then run the graph',
  'inspector.empty.noSelection': 'Select a node or segment to inspect',
  'inspector.empty.noSelectionHint': 'Shift-select two nodes to compare a segment',
  ...
  'inspector.backward.disabled': 'Turn on Capture gradients in Settings and re-run',
  ...
  'runs.reattach.message': 'This tab stops showing events from run {current}. That run keeps running.',
  ...
  'nodeDetail.noParams': 'No parameters',
  ...
  'nodeDetail.code.contract': 'def run(inputs, params) -> dict — one key per connected port (in1, in2, ...). Return {"out1": value}, or a bare value for out1.',
  'nodeDetail.code.security':
    'This check limits which libraries a script may import. Scripts run in the CodefyUI process with your permissions, and one that passes here can still be refused at run time. Only run code you trust.',
  ...
  'nodeDetail.code.portType': '{port} type',
  ...
  'nodeDetail.captures.notRun': 'Nothing captured yet',
  'nodeDetail.captures.notRunHint': 'Turn on Record node outputs in Settings, then run the graph',
  'nodeDetail.captures.recordingOff':
  ...
  'nodeDetail.captures.noSummary': 'Not recorded',
  'nodeDetail.stats.notRun': 'No statistics yet',
  ...
  'nodeDetail.stats.sampledNote':
    'Mean, std, quantiles and the histogram use this sample; counts, min/max and class balance are exact.',
  ...
  // Source Control (the sidebar's fifth tab). Parts 2 and 3 of the track:
  // status, stage / unstage / discard, commit, the commit identity, and then
  // branches, remotes, stashes, the network operations and merge resolution.
  // The history list and the diff view arrive with their own strings later, so
  ...
  'git.action.sync': 'Sync (pull, then push)',
  'git.action.publish': 'Publish Branch',
  'git.action.mergeRemote': 'Merge remote changes',
  'git.action.abortMerge': 'Abort Merge',
  ...
  'git.branch.namePrompt': 'Branch name',
  'git.branch.invalid': 'Not a valid branch name',
  'git.branch.switch': 'Switch to {name}',
  'git.branch.current': 'Current',
  'git.branch.rename': 'Rename',
  'git.branch.delete': 'Delete',
  'git.branch.renamePrompt': 'New name for {name}',
  'git.branch.deleteConfirm': 'Delete branch {name}?',
  'git.branch.forceDeleteConfirm': '{name} has unmerged commits. Delete anyway?',
  'git.branch.remote': 'Remote branches',
  'git.remote.add': 'Add Remote...',
  'git.remote.namePrompt': 'Remote name',
  'git.remote.urlPrompt': 'Remote URL (https:// or git@...)',
  'git.remote.invalidUrl': 'Use an https:// or SSH URL',
  'git.remote.changeUrl': 'Change URL',
  'git.remote.remove': 'Remove',
  'git.remote.removeConfirm': 'Remove remote {name}?',
  'git.remote.empty': 'No remote yet.',
  'git.stash.messagePrompt': 'Stash message (optional)',
  'git.stash.pop': 'Pop',
  ...
  'git.op.create_branch': 'create branch',
  'git.op.checkout': 'switch branch',
  'git.op.rename_branch': 'rename branch',
  'git.op.delete_branch': 'delete branch',
  'git.op.add_remote': 'add remote',
  'git.op.set_remote_url': 'change remote URL',
  'git.op.remove_remote': 'remove remote',
  'git.op.stash_push': 'stash',
  'git.op.stash_pop': 'pop stash',
  'git.op.stash_apply': 'apply stash',
  'git.op.stash_drop': 'drop stash',
  'git.op.abort_merge': 'abort merge',
  'git.op.resolve': 'resolve conflict',
  'git.op.fetch': 'fetch',
  'git.op.pull': 'pull',
  'git.op.push': 'push',
  'git.op.sync': 'sync',
  'git.op.publish': 'publish',
  ...
  'git.merge.ours': 'Keep mine',
  'git.merge.theirs': 'Take incoming',
  'git.merge.mark': 'Mark resolved',
  ...
  'git.toast.upToDate': 'Already up to date',
  'git.toast.fetched': 'Fetched',
  'git.toast.synced': 'Synced',
  'git.toast.published': 'Published {branch} to {remote}',
  ...
  'git.error.authRequired': 'The server computer has no saved credentials for this remote.',
  'git.error.authRequiredHint': 'Run gh auth login (HTTPS) or set up an SSH key, then retry.',
  'git.error.network': 'Could not reach the remote.',
  'git.error.nonFastForward': 'The remote has commits you do not have. Pull first.',
  'git.error.diverged': 'Local and remote branches have diverged.',
  'git.error.conflict': 'Merge conflicts: resolve the files under Merge Changes.',
  'git.error.dirtyTree': 'Uncommitted changes would be overwritten. Commit or stash them first.',
  'git.error.noUpstream': 'This branch is not published yet.',
  'git.error.noRemote': 'Add a remote first.',
  'git.error.branchExists': 'A branch with that name already exists.',
