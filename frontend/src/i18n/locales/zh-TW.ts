...
const zhTW: Record<TranslationKey, string> = {
  ...
  'toolbar.export.title': '將這張圖存成可重用的預設組合，列在「節點」分頁',
  ...
  // Sidebar: Graphs tab
  'graphs.search': '搜尋已儲存的圖表...',
  'graphs.refresh': '重新整理圖表清單',
  'graphs.loading': '載入圖表中...',
  'graphs.empty': '沒有已儲存的圖表',
  'graphs.noMatch': '沒有符合「{query}」的圖表',
  ...
  // 內嵌張量表格編輯器（節點設定）。{mode} 會以 <code> 呈現 value_mode 這個
  // 參數名稱：它是這句話裡唯一需要讀者回頭去面板上找的字，不該讀起來像散文。
  'tensorGrid.needExplicit': '將 {mode} 設為 explicit 才能編輯這些數值。',
  'tensorGrid.tooLarge':
    '{total} 個元素太多，無法在這裡編輯（上限 {max}）—— 請將 {mode} 設為 random、zeros、ones 或 arange。',
  'tensorGrid.fillZero': '填 0',
  'tensorGrid.fillOne': '填 1',
  'tensorGrid.random': '隨機',
  'tensorGrid.cells': '{count} 格',
  // dim 在每個語言都維持張量 API 自己的用字：它標示的下拉選單就緊貼在
  ...
  'results.title': '執行紀錄',
  'results.training': '訓練',
  'results.trainingConfig': '訓練設定',
  'results.trainingEmpty': '尚無訓練資料。',
  'results.clear': '清除',
  'results.empty': '執行管線後會顯示輸出。',
  ...
  'layersEditor.import.title': '匯入架構檔',
  'layersEditor.export.title': '匯出為 JSON',
  'layersEditor.empty': '拖曳左側的層級到這裡開始',
  ...
  'layersEditor.import.noContent': '此檔案沒有層級或 SequentialModel 節點。',
  ...
  'layersEditor.autoLayoutTitle': '依連線順序由上而下排列',
  'layersEditor.category.io': '輸入/輸出',
  'layersEditor.category.merge': '合併',
  'layersEditor.validation.cycle': '圖中有循環',
  'layersEditor.validation.noInput': '圖必須剛好有一個 Input 節點',
  'layersEditor.validation.noOutput': '圖必須剛好有一個 Output 節點',
  'layersEditor.validation.inputNeedsPort': 'Input 節點至少要有一個連接埠',
  'layersEditor.validation.outputNeedsPort': 'Output 節點至少要有一個連接埠',
  'layersEditor.validation.dupInputPorts': 'Input 連接埠名稱不能重複',
  'layersEditor.validation.dupOutputPorts': 'Output 連接埠名稱不能重複',
  'layersEditor.validation.outputEdgeCount':
    'Output 連接埠「{port}」必須剛好有 1 條連入的線（目前 {count} 條）',
  'layersEditor.validation.inputPortUnused': 'Input 連接埠「{port}」沒有被使用',
  'layersEditor.validation.layerEdgeCount':
    '層級「{layer}」必須剛好有 1 條連入的線（目前 {count} 條）',
  'layersEditor.validation.unreachable': '節點「{layer}」無法從 Input 到達',
  'layersEditor.validation.cannotReachOutput': '節點「{layer}」無法連到 Output',
  'layersEditor.import.emptyGraph': '圖是空的或格式不正確',
  'layersEditor.category.convolution': '卷積',
  'layersEditor.category.normalization': '正規化',
  'layersEditor.category.pooling': '池化',
  'layersEditor.category.regularization': '正則化',
  'layersEditor.category.linear': '線性',
  // LSTM／GRU／RNNCell 的節點說明都寫「遞迴層」，標題要跟著同一個詞；
  // 「循環」在這個編輯器裡已經是驗證訊息裡 cycle 的用字。
  'layersEditor.category.recurrent': '遞迴',
  'layersEditor.category.attention': '注意力',
  'layersEditor.category.utility': '工具',
  'layersEditor.category.activation': '激活函數',
  'layersEditor.port.add': '新增連接埠',
  'layersEditor.port.remove': '移除',
  'layersEditor.port.namePlaceholder': '連接埠名稱',
  'layersEditor.port.duplicate': '連接埠名稱重複',
  'layersEditor.port.list': '連接埠',
  ...
  'results.clickToExpand': '顯示錯誤詳情',
  'results.clickToHighlight': '高亮這個節點',
  'results.imageAlt': '節點輸出影像',
  'results.videoAlt': '節點輸出影片',
  ...
  'inspector.emptyPorts': '沒有連接埠',
  'inspector.dataExpired': '執行資料已過期 — 重新執行以擷取',
  'inspector.dataExpiredBackward': '執行資料已過期 — 請在設定中開啟「擷取梯度」後重新執行',
  'inspector.tensorExpired': '已過期',
  'inspector.noTensors': '沒有張量',
  'inspector.tensor.module': '模組',
  'inspector.tensor.params': '參數量',
  'inspector.empty.notRun': '尚未擷取資料',
  'inspector.empty.notRunHint': '請在設定中開啟「錄製節點輸出」，然後執行圖表',
  'inspector.empty.noSelection': '選取節點或段落以檢視',
  'inspector.empty.noSelectionHint': 'Shift 選兩個節點可比較段落',
  ...
  'inspector.backward.disabled': '請在設定中開啟「擷取梯度」後重新執行',
  ...
  'nodeDetail.code.contract':
    'def run(inputs, params) -> dict — 每個有連線的連接埠一個鍵（in1、in2…）。回傳 {"out1": 值}，或直接回傳單一值當作 out1。',
  'nodeDetail.code.security':
    '這項檢查只限制腳本能匯入哪些函式庫。腳本以你的權限在 CodefyUI 行程內執行，通過檢查的腳本仍可能在執行時被拒絕。只執行你信任的程式碼。',
  ...
  'nodeDetail.code.portType': '{port} 型別',
  ...
  'nodeDetail.captures.notRun': '尚未擷取資料',
  'nodeDetail.captures.notRunHint': '請在設定中開啟「錄製節點輸出」，然後執行圖表',
  'nodeDetail.captures.recordingOff': '「錄製節點輸出」已關閉 — 請在設定中開啟後重新執行，即可擷取數值',
  'nodeDetail.captures.summaryTitle': '上一次執行的形狀',
  'nodeDetail.captures.noSummary': '尚未擷取',
  'nodeDetail.stats.notRun': '尚無統計',
  ...
  'nodeDetail.stats.sampledNote':
    '平均、標準差、分位數與直方圖來自這個取樣；各項計數、最小/最大值與類別分佈為精確值。',
  ...
  'nodeDetail.docs.noDescription': '沒有說明',
  'nodeDetail.docs.params': '參數',
  'nodeDetail.docs.noParams': '無',
  'nodeDetail.docs.inputs': '輸入連接埠',
  'nodeDetail.docs.outputs': '輸出連接埠',
  'nodeDetail.docs.noPorts': '無',
  ...
  'packs.toast.busy': '已有另一個安裝進行中。',
  'packs.toast.pluginBusy': '外掛安裝進行中，請於完成後再試。',
  'packs.toast.openPluginCenter': '開啟外掛中心',
  'packs.toast.needsCli': '此套件無法在程式內安裝，請執行：{command}',
  'packs.toast.restartRetry': '安裝因伺服器已載入的套件而中止，請在套件中心重新啟動伺服器以完成安裝。',
  'packs.toast.blocked': '請先安裝 {pack}。',
  'packs.toast.restartRefusedRunning': '有圖正在執行，請先停止再安裝。',
  'packs.toast.restartRefusedPending': '已有重新啟動進行中，請等待伺服器恢復。',
  'packs.toast.devRestart':
    '此套件需重新啟動伺服器，cdui dev 模式無法自動處理，請使用套件中心顯示的指令。',
  'packs.toast.inProgress': '套件安裝中，可在套件中心查看進度。',
  ...
  'packs.title': '套件中心',
  'packs.subtitle': 'LLM 節點的選用模型與函式庫',
  'packs.close': '關閉套件中心',
  ...
  'packs.unsupported': '此伺服器不支援套件中心，請更新 CodefyUI 後重新啟動。',
  ...
  'packs.catalog.sentence-embeddings.desc':
    'TextEmbedding 與 WordVector 節點之 sentence-transformers 及四個小型嵌入模型',
  'packs.catalog.word-vectors.title': '詞向量（GloVe）',
  'packs.catalog.word-vectors.desc':
    'WordVector 節點之 GloVe-50d 詞向量表擴增（40 萬詞）',
  'packs.catalog.rag.title': 'RAG 檢索堆疊',
  // 兩句都不再重複下一列已經寫過的事：相依列會帶著即時狀態說「需先安裝：
  // 句向量模型」，GPU 說明再往下兩列的按鈕就寫著「安裝並重新啟動」。
  'packs.catalog.rag.desc': 'HFTextGenerate 節點之本機生成模型 Qwen2.5-0.5B-Instruct',
  'packs.catalog.gpu-torch.title': 'GPU 版 PyTorch',
  'packs.catalog.gpu-torch.desc': '切換為符合本機 GPU 的 CUDA/ROCm 版 PyTorch',
  ...
  'packs.dependsOnLabel': '需先安裝：',
  'packs.dependsOnMissing': '請先安裝 {pack}',
  'packs.selectSomething': '請至少勾選一個項目',
  'packs.installSelected': '安裝選取項目',
  'packs.cancel': '取消安裝',
  'packs.cancelling': '取消中...',
  'packs.remoteDisabled': '僅能在執行伺服器的電腦上安裝。',
  'packs.sessionExpired': '伺服器已重新啟動，請重新整理頁面後再試。',
  ...
  'packs.activity.idle': '目前沒有安裝作業。',
  'packs.activity.idleHint': '關閉視窗後下載仍會繼續。',
  'packs.activity.job': '正在安裝 {pack}',
  'packs.activity.step': '步驟 {index}：{label}',
  'packs.activity.step.pip': '安裝 Python 套件',
  'packs.activity.step.download': '下載 {item}',
  'packs.activity.step.convert': '轉換 {item}',
  'packs.activity.step.verify': '驗證安裝結果',
  ...
  'packs.activity.restartAndInstallNote':
    '伺服器將重新啟動並安裝 Python 套件；模型須於重啟後再執行一次安裝下載。',
  ...
  'packs.gpu.none': '未偵測到 GPU，目前為 CPU 版 PyTorch。',
  'packs.gpu.installed': '目前版本：{variant}',
  'packs.gpu.recommended': '建議版本：{variant}',
  'packs.gpu.variant': 'PyTorch 版本',
  'packs.item.converting': '轉換中（僅需一次）',
  'packs.gpu.alreadyOptimal': 'GPU 加速已內建於預設的 PyTorch 版本。',
  'packs.gpu.restartNote': '圖執行期間無法重新啟動。',
  'packs.gpu.restartConfirm': '安裝 {variant} 並重新啟動伺服器？',
  'packs.gpu.installRestart': '安裝並重新啟動',
  'packs.gpu.devMode':
    'cdui dev 模式下伺服器無法自行重新啟動，請在後端終端機執行下列指令後重新啟動：',
  'packs.gpu.notYet': '無法在程式內切換 PyTorch 版本，請停止伺服器後執行：',
  'packs.gpu.noCommand': '伺服器未提供安裝指令，請參考 README 的 GPU 安裝步驟。',
  'packs.manualCommand': '手動指令',
  'packs.copy': '複製指令',
  'packs.copied': '已複製到剪貼簿。',
  'packs.copyFailed': '無法複製，請手動選取文字複製。',
  ...
  'packs.toast.missingPack': '此次執行需要 {pack} 套件。',
  ...
  'settings.plugins.unsupported': '此伺服器不支援',
  ...
  'pluginCenter.unsupported': '此伺服器不支援外掛中心，請更新 CodefyUI 後重新啟動。',
  ...
  'git.history.copied': '已複製 {sha}',
  'git.history.copyFailed': '無法複製提交 ID。',
  'git.diff.title': '變更：{path}',
  'git.diff.view': '檢視方式',
  'git.diff.unified': '單欄',
  'git.diff.split': '並排',
  'git.diff.close': '關閉',
  'git.diff.loading': '正在載入變更...',
  'git.diff.scope.worktree': '未暫存的變更',
  'git.diff.scope.index': '已暫存的變更',
  ...
  'git.gdiff.edgesAdded': '新增 {count} 條連線',
  'git.gdiff.edgesRemoved': '移除 {count} 條連線',
  'git.gdiff.positionsMoved': '{count} 個節點位置移動',
  'git.gdiff.more': '還有 {count} 項',
  'git.gdiff.noLogicChange': '邏輯無變更',
  'git.gdiff.unparseable': '無法解析為圖表',
