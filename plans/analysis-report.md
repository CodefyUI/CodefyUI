...
│  └────────────────────────────────────────────────────────────────┘  │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  Node Packages (63 nodes, 13 categories)                        │  │
│  │  CNN(9) | RNN(2) | Transformer(3) | RL(3) | Data(6)             │  │
│  │  Training(5) | IO(8) | Control(1) | Dataflow(3) | Utility(8)    │  │
...
### 2.4 內建節點清單 (63 個，13 類別)

> ★ = StatefulModuleMixin（持續化權重） · ✦ = 含 Verbose Step Trace · ⚙ = 2026-04 新增
...
| **Utility** | Linear★, Embedding★, Flatten, Concat, Reshape, SequentialModel, Visualize, GraphModel | 8 |
| **Tensor Ops** | Add, MatMul, Mean, Multiply, Permute, Reshape, Softmax✦, Split, Squeeze, Stack, TensorCreate / Unsqueeze | 11 |
| **Training** | Optimizer, Loss, LRScheduler, TrainingLoop, ⚙ BackwardOnce | 5 |
| **IO** | ImageReader, ⚙ ImageWriter, ⚙ ImageBatchReader, FileReader, ModelLoader, ModelSaver, Checkpoint, Inference | 8 |
| **Data** | Dataset, DataLoader, Transform, ⚙ TensorInput, HuggingFaceDataset, KaggleDataset | 6 |
...
| **排程執行** | 無 | Cron、事件觸發 | 中 |
| **節點生態** | 63 個內建（13 類別）+ Custom Node Manager GUI | 社群市場、包管理器 | 中（管理工具已有，無遠端市集） |
...
5. **MIT 授權 + 開源 + 自建社群**：在教育場景無收費壓力，能自然累積使用者基數與社群節點。
...
| 功能 | CodefyUI | ComfyUI | n8n | Langflow | Dify | Kubeflow | Airflow |
|------|----------|---------|-----|----------|------|----------|---------|
| 內建節點數 | 63 | ~50 核心 | 400+ | ~100 | ~80 | ~30 | 100+ Operator |
...
| 範例工作流 | 19（CI 驗證） | 2.5M（社群） | 數百 | 數十 | 數十 | 數十 | 數十 |
...
4. **節點數量是中位** — 63 個比 ComfyUI 核心多，但遠少於 n8n（400+）；不過 CodefyUI 節點都有 type safety + 教育性，品質 > 數量。
